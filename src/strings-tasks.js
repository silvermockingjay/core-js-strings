...
function isPalindrome(str) {
  ...
  for (let i = 0; i < copy.length; i += 1) {
    if (!chars.includes(copy.charAt(i))) {
      word += copy.charAt(i);
    }
  }
...
function reverseWords(str) {
  ...
  if (copy.includes(' ')) {
    words = copy.split(' ');
    const reversedWords = words.map((item) => reverseItem(item));
    return reversedWords.join(' ');
  }
  return reverseItem(copy);
}
...
function extractNameFromTemplate(value) {
  const myRegEx = /Hello, (\w+) (\w+)!/g;
  const result = myRegEx.exec(value);
  result.shift();
  return result.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (/[A-Z]/.test(str.charAt(i))) {
      const asciiValue = str.charAt(i).charCodeAt(0);
      let nextAsciiValue = asciiValue + 13;
      if (nextAsciiValue > 90) nextAsciiValue -= 26;
      encodedString += String.fromCharCode(nextAsciiValue);
    } else if (/[a-z]/.test(str.charAt(i))) {
      const asciiValue = str.charAt(i).charCodeAt(0);
      let nextAsciiValue = asciiValue + 13;
      if (nextAsciiValue > 122) nextAsciiValue -= 26;
      encodedString += String.fromCharCode(nextAsciiValue);
    } else {
...
function getCardId(value) {
  ...
  return cardDeck.findIndex((item) => item === value);
}
